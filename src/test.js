...
let P = bigP_generation()

let G = g_generation(P)

let privatekey = privateKey_gen(P,G)

let publickey = publicKey_gen(P,G,privatekey)

let num = Array(40).fill().map(() => Math.round(Math.random() * 40))

console.log(num)

console.log(num.reduce((prev,now)=>{
    return prev + now
}))

let enc = num.map((v)=>{
    return encrypt(v, P, G, publickey)
})

console.log(enc)

let final = enc.reduce((prev,curr)=>{
    return add_encrypted(prev,curr)
})

console.log(decrypt(final,P,G,privatekey))